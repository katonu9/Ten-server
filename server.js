...
class Game {
  constructor(room) {
    this.room = room;
    this.version = 0;
    this.phase = "lobby"; // lobby -> draw -> trump -> playing -> over
    this.players = [null, null, null, null]; // {name, token, bot:false} | {name, bot:true}
    this.hostToken = null;
    this.reset();
  }
  ...
  startHand() {
    ...
    // the pregame draw, performed by the table: everyone draws one card
    let deck = shuffle(freshDeck());
    let results;
    for (;;) {
      results = [0, 1, 2, 3].map((seat) => ({ seat, card: deck.pop() }));
      const hiA = Math.max(...results.filter((r) => TEAM_OF[r.seat] === "A").map((r) => r.card.rank));
      const hiB = Math.max(...results.filter((r) => TEAM_OF[r.seat] === "B").map((r) => r.card.rank));
      if (hiA !== hiB) break;
      deck = shuffle(freshDeck()); // dead-even top cards: redraw
    }
    this.drawResults = results;
    const highest = results.slice().sort((a, b) => b.card.rank - a.card.rank)[0];
  ...
  setTrump(suit) {
    ...
    this.say(this.seatName(this.picker) + " chose " + SUIT_NAME[suit] + " as trump. " + this.seatName(this.turn) + " leads the first round.");
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && path === "/state") {
    ...
    const seat = room.tokens.get(url.searchParams.get("token"));
    return json(res, 200, room.game.snapshot(seat != null ? seat : null));
  }
  ...
  req.on("end", () => {
    ...
    if (path === "/create") {
      ...
      game.players[0] = { name, bot: false };
      game.hostToken = token;
      game.say("Waiting for players... share code " + code);
      const entry = { game, tokens: new Map([[token, 0]]), created: Date.now() };
      rooms.set(code, entry);
      return json(res, 200, { room: code, token, seat: 0 });
    }
    ...
    if (path === "/join") {
      ...
      const seat = [1, 2, 3].find((s) => !game.players[s]);
      if (seat == null) return json(res, 400, { err: "Room is full." });
      const token = newToken();
      const name = String(data.name || "Player").slice(0, 14) || "Player";
      game.players[seat] = { name, bot: false };
      room.tokens.set(token, seat);
      game.say(name + " joined.");
      return json(res, 200, { room: game.room, token, seat });
    }

    const seat = room.tokens.get(data.token);
    if (seat == null) return json(res, 403, { err: "Bad token." });
